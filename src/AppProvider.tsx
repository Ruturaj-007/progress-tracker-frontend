...
import API_BASE_URL from "./services/api"; // <-- Added import
...
const reducer = (state: AppState, action: Action): AppState => {
  switch (action.type) {
    ...
    case GET_TASK: {
      ...
      return {
        ...state,
        tasks: { ...state.tasks, [action.payload.taskListId]: updatedTasks },
      };
    }
    case UPDATE_TASK:
      return {
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const api: AppContextType["api"] = {
    ...
    getTaskList: async (id: string) => {
      const response = await axios.get<TaskList>(`${API_BASE_URL}/task-lists/${id}`, jsonHeaders);
      dispatch({ type: GET_TASKLIST, payload: response.data });
    },
    createTaskList: async (taskList) => {
      const response = await axios.post<TaskList>(`${API_BASE_URL}/task-lists`, taskList, jsonHeaders);
    ...
    deleteTask: async (taskListId, taskId) => {
      await axios.delete(`${API_BASE_URL}/task-lists/${taskListId}/tasks/${taskId}`, jsonHeaders);
